...
Thread.prototype.updGrab = function() {
    ...
    if (this.rGrab <= 0.5) { this.rHalf = this.rGrab/0.5; }
    else { this.rHalf = 1-(this.rGrab-0.5)/0.5; }
...
Machine.prototype.incrLoad = function() {
    ...
    if (this.rLoad > 0.15) {
        ...
        nub = this.arrNubs[0]; if (!nub.hasEntered) nub.enter(); // Double check that the first one has entered, it skipped once
    }
    if (this.rLoad > 0.85) {
...
Nub.prototype.drop = function() {
    ...
    if (vxa > THROW_SPD_MAX) {
        ...
        if (this.dx == 0) this.velX = (Math.random() > 0.5 ? -1 : 1) * THROW_SPD_MIN; // don't divide by zero
    ...
    if (vya > THROW_SPD_MAX) {
        ...
        // Since user wasn't moving, need to use our own dx.
        if (this.dy == 0) this.velY = (Math.random() > 0.5 ? -1 : 1) * THROW_SPD_MIN; // don't divide by zero
...
Nub.prototype.redraw = function() {
    ...
    for (var i = 1; i < TRAIL_PTS_LIM; i++) {
        ...
        this.cv.quadraticCurveTo(
            this.m.xo + x1, this.m.yo + y1, // First control point
            this.m.xo + xh1, this.m.yo + yh1 // End anchor point
        );
        // increment points
